	...
	while(1)
	{
		...
		while(++i < session->num_philos)
		{
			...
			if(current_time > philos[i].time_to_die)
				return (session->program_status = 'd', print_message(MSG_DEAD, &philos[i], philos[i].session), pthread_mutex_unlock(&(session->die_lock)), NULL);
...
void* thread_function(void* arg)
{
	...
	while(1)
	{
		...
		take_forks(philo);
		philo->eat_count++;
		if (philo->session->num_philo_must_eat > 0 && philo->eat_count > philo->session->num_philo_must_eat)
		{
...
t_philo	*create_philos(int count, t_session *session)
{
	...
	while (++ i < count)
	{
		philos[i] = (t_philo){&session->pid[i], i + 1, 0, 
			get_time_milisec() + session->time_to_die, &session->forks[(i + 1) % count], 
			&session->forks[i], session, pthread_mutex_init(&(philos[i].eat_lock), NULL), 0};
